...
These models are built around a simple framework - making a "best fit" line through your data. You've certainly seen an x-y scatterplot with a fitted line plotted through the middle. This is actually a simple regression model. Our models fit functions that predict the value of Y for a given X. Functionally, it minimizes the distance between the observations (the error) and the best fit line.
...
Let's say we are interested in food system planning, and we want to be able to predict the yield (weight per hectre) for several crop types - maize, wheat, etc.,
...
It would stand to reason that the model "knowing" what country and crop type you're predicting would help. Some countries are probably easier than other to grow certain kinds of crops. Our exploratory analysis told us that yields vary a lot across crop and country.

Let's add a "categorical" variable to our model - `Item` - aka crop type.
...
Let's create a new regression, called `reg2`. In this regression - each coefficient represents the unit change in crop yield associated with being Maize relative to being in Cassava (the hold out category).

Adding this fixed effects made our R-Squared go wayyyy up - if we know the rainfall, crop and pesticide application, we can explain almost 60% of the variance in crop yields. Pretty good - but again - R-squared is not the metric we care about the most.
...
We saw there is a very strong relationship between yield and `yield_last_year` - what if we took the country out of the model, and put this in.... wow. That's a very strong predictor and a very high R-squared.

You can see that it knocks rainfall, temperature and pesticides out of the model in terms of significance (p-values). This is because these things are "co-linear" with `yield_last_year`. Co-linearity is a sign that the coefficient values are not very reliable when these variables are in the model together. This is a major way that prediction and explanation are very different when you use OLS models - we are violating some important statistical assumptions in exchange for predictive ability.
...
Binary logistic regression, or "logit" is different from OLS modeling because it is designed to estimate binary outcomes. Specifically, if you have an outcome with two levels (say 0 and 1), it can estimate the odds that your combination of independent variables predicts the dependent variable to be a "1".
